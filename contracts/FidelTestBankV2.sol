...
contract FidelTestBankV2 is Ownable {

    IERC20 public FIDELTEST;//Fidel Token contract
    ...
    // Stake token
        ...
        //and then calculate the calcRewards() for each individual deposit.
    ...
    function migrateToNewContract(address _newContractAddress) public {
        ...
        // Sends the staked tokens to new contract
        FIDELTEST.transfer(_newContractAddress, stakingBalance[msg.sender]);

    } 

    //The contract owner updates the new contract mapping with the migrating staker info
    function allowMigration(address staker_account, uint _balance, uint _time) public onlyOwner{
        stakingBalance[staker_account]= _balance;
        stakingTime[staker_account]= _time;
    }
